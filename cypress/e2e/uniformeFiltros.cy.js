describe('Uniforme API Tests', () => {
  ...
  it('Crear nueva salida para el Uniforme(Test 1 )', () => {
    const newSalida = {
        uniforme_ID: id,
        salida:{
            fecha: new Date().toISOString(),
  ...
  it('Crear nueva salida para el Uniforme(Test 2 )', () => {
    const newSalida = {
        uniforme_ID: id,
        salida:{
            fecha: new Date().toISOString(),
    ...
    it('Lista de entradas del Uniforme(Test)', () => {
      cy.request('GET', 'http://localhost:3000/ApiMinig/Uniformes/AllEntradas/'+id._id)
        .then((response) => {
          expect(response.status).to.eq(200);
          expect(response.body).to.have.property('status',true);
      });
    });
    it('lista de Salidas del uniforme(Test)', () => {
      const objeto = {
